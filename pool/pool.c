...
void get_balls_coord(void)
{
	...
	Y0 = 5 * (xrnd() % ((TYMAX-TYMIN)/5)) + TYMIN + 1;
	...
	do 
		Y1 = 5 * (xrnd() % ((TYMAX-TYMIN)/5)) + TYMIN + 1;
...
again:
	...
	PrintStr(47, 0, "move crosshair (s=left,d=right,x=down,e=up) then hit <cr>");
	
	while (MoveCH(CrtIn()) == 0)
		;
...
recompute:
	...
	do
	{
		...
		else if (status == STOPPED)
		{
			...
			ClearTopLine();
			PrintStr(47,0,"ball stopped, continue? (y/n):");
			SetCursor();

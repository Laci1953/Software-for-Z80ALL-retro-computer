/***************************************************************************
   ...
   HiTech C porting , Z80ALL version, Game Save/Load by Ladislau Szilagyi
...
void clr_screen(void)  { 
    putchar( ESC ); putchar( 'H' );	//home
    putchar( ESC ); putchar( 'J' );	//erase to end-of-screen
}  
...
void wipedice(void) {
	...
	gotoxy(40,11); blanks(18);	 /* erase dice roll messages */
	gotoxy(8,11); blanks(18);	
}
...
int xme, xyu;
	...
	xme = 40; xyu = 8;    /* decide which half to show the values in */ 
	if (point[1].x > 30) { 
		xme = 8; 
		xyu = 40; 
	}
...
void msg(char* p) {
	gotoxy(0,23); blanks(64);
	gotoxy(0,23); putstr(p); 
}

void debug(char* p) {
	gotoxy(0,22); blanks(64);
	gotoxy(0,22); putstr(p); 
}
...
void notyrcube(void) {
	gotoxy(59,20); blanks(5);
	gotoxy(59,21); blanks(5);
} 

void notmycube(void) {
	gotoxy(59,1); blanks(5);
	gotoxy(59,2); blanks(5);
} 
...
void notbarcube(void) {
	gotoxy(30,11); putstr(" BAR ");
} 

void mycube(int value) {
	notbarcube(); gotoxy(59,1); putstr("CUBE");
	gotoxy(59,2); 
...
void yrcube(int value) {
	notbarcube; gotoxy(59,20); putstr("CUBE");
	gotoxy(59,21); sprintf(buzzard,"[%d]",value); 
	printf("%-5s",buzzard);
